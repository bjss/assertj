...
class DoesNotMatchPredicateTest implements WithAssertions {
  ...
  void should_not_match_predicate() {
    assertThat(yoda).doesNotMatch(x -> x.lightSaberColor.equals("Blue"));
  }

  @Test
  void should_not_match_predicate_with_description() {
    assertThat(yoda).doesNotMatch(x -> x.lightSaberColor.equals("Blue"), "has blue light saber");
  }

  @Test
  void should_fail_if_object_does_match_predicate() {
  ...
  @Test
  void should_fail_if_object_does_match_predicate_and_use_predicate_description_in_error_message() {
    assertThatExceptionOfType(AssertionError.class).isThrownBy(() -> assertThat(yoda).as("check light saber")
      ...
      .withMessage(format("[check light saber] %n" +
        "Expecting actual:%n" +
        "  Yoda the Jedi%n" +
        "not to match 'has red light saber' predicate."));
  }
